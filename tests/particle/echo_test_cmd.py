...
-mimc_bayesian {bayesian} ".format(bayesian="{bayesian}", TOL="{TOL}", seed="{seed}")
    ...
    if args.tries == 0:
        cmd_single = "python3 " + base_1 + " -mimc_verbose 1 -db False "
        print(cmd_single.format(seed=0, bayesian=False, TOL=0.001))
        cmd_single = "python " + base_2 + " -mimc_verbose 1 -db False "
        print(cmd_single.format(seed=0, bayesian=False, TOL=0.001))
    else:
        cmd_multi_1 = "python3 " + base_1 + " -mimc_verbose 0 -db True -db_tag {tag} "
        cmd_multi_2 = "python3 " + base_2 + " -mimc_verbose 0 -db True -db_tag {tag} "
        TOL = 1e-5
